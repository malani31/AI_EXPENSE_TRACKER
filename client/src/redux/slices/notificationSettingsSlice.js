...
export const fetchNotificationSettings= createAsyncThunk(
    ...
    async (userId,{rejectWithValue})=>{
        try {
            ...
            return rejectWithValue(error.response?.data?.message || "Eror during fetchingnotificationsettings");
        }
    }
);
...
export const updateNotificationSettings= createAsyncThunk(
    ...
    async ({userId,settings},{rejectWithValue})=>{
        try {
            const {data}=await API.get(`/notification-settings/${userId}`,settings);
            return data;
        } catch (error) {
            return rejectWithValue(error.response?.data?.message || "Eror during updating notificationsettings");
        }
    }
);
